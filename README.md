...
- **Advanced Filtering & Export**: Powerful server-side filtering (Amount range, Date, Category, **Slip Number range** with numeric CAST — single value = exact match, two values = `BETWEEN`) and data export to XLSX/PDF. Reports table mirrors the Donation List (colored Mode column, humanized status, DD/MM/YYYY dates everywhere including Excel & PDF).
- **Installment History Toggle**: Donation list returns an `installmentCount` per donation via a correlated subquery; the row-expand eye icon only appears for donations that actually have installment history, so completed one-shot donations stay clean.
- **Announcement Donor Scope**: `/admin/donors?requireMobile=true` restricts results to donors with a non-empty `mobileNumber`. The Announcement (WhatsApp chat) page passes this flag so only contactable donors appear in the list.
