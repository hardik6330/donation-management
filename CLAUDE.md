...
- **Installment History Toggle:** Donation list query (`/admin/donations`) returns an `installmentCount` per row via a correlated subquery on `DonationInstallments`. The eye/expand button in `DonationList` only renders when `installmentCount > 0`, so donations with no payment history don't show an empty expander.
- **Reports Module:** Custom Reports table mirrors the Donation List (Mode column with `getPaymentModeColor`, status text humanized via `replace(/_/g, ' ')`, dates rendered as DD/MM/YYYY via `toLocaleDateString('en-GB')` in table, Excel, and PDF exports). Status filter offers only the three real lifecycle states from `donationStatuses` (Completed, Partially Paid, Pay Later) — no Pending/Failed.
- **Announcement Donor Scope:** `/admin/donors` accepts `requireMobile=true`; when set, the controller restricts `donorWhere.mobileNumber` to non-null, non-empty values. The Announcement page passes this flag via `useDropdownPagination`'s `additionalParams` so only contactable donors appear in the WhatsApp chat list.
...
- **Filtering:** `FilterSection` component with inline typing search (direct search in dropdown trigger). Server-side search for paginated dropdowns. `buildSearchFilter()` for generic multi-field search. `buildDonationFilter()` builds complex Sequelize where clauses. Donation queries support a numeric **slip number range** via `slipNoFrom`/`slipNoTo` (CAST UNSIGNED): `slipNoFrom` alone = exact match, both = `BETWEEN`, `slipNoTo` alone = `<=`. Implemented in `buildDonationFilter` and as the `bySlipNoRange` scope on the Donation model (used by `/admin/donations`).
